      ...
      switch(cmd){
      	            ...
      	            case "eliminar":
      		    	 ...
      		    	 case "abrir":    
      		    	     ...
      		    	     if(a.hasClass("medio"))      		    	     	 
      		    	         openMedios( function(){ dialog.show("bye") })
                  ...
                  break; 	
      		    	 ...
      		    	 case "share":
      		    	   ...
      		    	   var a = $(this);
      		    	   var file = a.attr("rel");
      		    	   var url = "http://serviaseo.com.co/f/" + file;
      		    	   var thumb = a.parent().parent().find("img").attr("src");
      		    	       thumb = thumb.replace("../../","http://serviaseo.com.co/");
      		    	       thumb = thumb.replace("img/","http://serviaseo.com.co/img/");


      		    	   $.ajax({

      		    	   	url : "core/php/app.short.php",
     	...
     	var ext = files[i].name.toString().split(".");
     	    ...
     	    if(!inArray(ext,exts))
     	    	if(files.length > 1)
     	    	 ...
     	    	 dialog.show("Solo puedes cargar imagenes, pdf's, zip's, rar's, mp3's");
     	    	 return;
...
function inArray(search,array){
 ...
 function sendFiles( data ){
   ...
   $("thumbs").addClass("loading");
 	...
 	$.ajax({
				url : '../panel/core/php/app.carga.php',
				type : 'POST',
				data : data,
				dataType : 'JSON',
				processData : false, 
			 ...
			 "404" : function(){ dialog.show("pagina no encontrada"); }
	...
	error : function(error){
			...
			console.log(error);
			dialog.show(error.responseText);
 ...
 function deleteFile(el,multiple){
	...
	if(preg){
	  ...
	  if(!multiple){
		...
		$.ajax({
			url : "../panel/core/php/app.eliminar.php",
			data : { _id : _id },
			statusCode : {
...
function loadMedios( callback ){
	...
	$.ajax({ 
		...
		url : "../panel/core/php/app.getInfo.php",
		data : {},
		dataType : "JSON",
		statusCode : {
		...
		var doc = rs.rs;	
		var opts = '<div class="opts"><a href="#abrir" title="ampliar" rel="#preview" class="prev" data-role="f/%src%"><i class="icon-search"></i></a><a href="#share" title="Compartir" rel="%url%"><i class="icon-link"></i></a><a href="#select" rel="%url%" title="seleccionar"><i class="icon-ok"></i></a><a href="#eliminar" title="eliminar"><i class="icon-remove"></i></a></div>';		
...
function getEtiqueta( mime ){
		...
		switch(mime){
			...
			return  "<div class='span3 thumbs' id='%_id%'>%opts%<img src='t/150/%src_%' alt='' /></div>";												
			...
			return "<div class='span3 thumbs file' id='%_id%'>%opts%<a href='f/%src_%' target='_blank'><img src='img/pdf.png' alt='' /></a></div>";									

			break;

			case "application/octet-stream":

			return "<div class='span3 thumbs file' id='%_id%'>%opts%<a href='f/%src_%' target='_blank'><img src='img/rar.png' alt='' /></a></div>";									
			...
			return { modelo : "<div class='span3 thumbs file'id='%_id%'>%opts%" +
			        "<a href='../../medios/%src_%' target='_blank'><img src='img/mp3.png' class='media' alt='' />'</a>" +
			        '<video controls class="hidden"><source src="f/%file%" type="audio/mpeg"></video>'+
			        "</div>" , 
			       file : true ,
			       type : "mp3"
...
function openShare(rs,thumb,file){
	...
	var server_name = "http://serviaseo.com.co";
   ...
   $(".medio_target").live("change", function(){	
	 ...
	 if(objs.length > 0)
   	  for(i=0; i < els.length ; i++){
   	  	 ...
   	  	 var text = "<div class='span2 thumbs' style='height: 80px; width: 80px; overflow:hidden'>" + optsz + "<img src='t/150/" + els[i] + "' alt='' /></div>";   	  	 	 
