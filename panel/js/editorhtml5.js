...
(function(wysihtml5) {
  ...
  wysihtml5.commands.pasteFromWord = {

    exec : function(composer, command, value){

        var msg = "<textarea style='width: 97%;overflow:hidden; <display: table;margin: 0 auto;' id='pasteFromWord' cols='4' rows='4' placeholder='Pega el texto aquí'></textarea>"
        var composer = composer;

        dialog.show( msg, { ask : true ,onAccept : function(){ 
               console.log($("#pasteFromWord").val());
               var text = html_entity_decode($("#pasteFromWord").val())
                          .replace(/<br>|<br\/>/g,"{{br}}");
...
function parseVideo(url){
    ...
    return (url.match("youtu"))  ? "http://www.youtube.com/embed/" + youtube_parser(url) : "//player.vimeo.com/video/" + vimeo_parser(url)
         ...
         var insertVideo = function(){
              ...
              var url = $("#wysihtml5-video").val();
              var video = url.match(/youtu|vimeo/);

              if(video){

                var videoWrap = "<div style='width:60% !important; min-width:320px !important; min-height:250px !important; position:relative'><iframe style='width:100%; position:absolute; height:100%' src='{{src}}'></iframe></div>";
                    video = videoWrap.replace(/\{\{src\}\}/, parseVideo(url));

                    editor.setValue( editor.getValue() + video);

              }
              else
                {
                    dlg("La URL del vídeo no es valida");                    
                    return false;
                }

         }

         var tam = "<select data-wysihtml5-dialog-field='className'><option value=''>Original</option>"
          +"<option value='wysiwyg-img-small'>Pequeño</option>"
          +"<option value='wysiwyg-img-medium'>Mediano</option>"
          +"<option value='wysiwyg-img-big'>Grande</option>"
          +"</select>";

         var msg = "<span style='display:block'>URL video:</span><br><input style='width:97%; display:table; margin:0 auto; float:none' type='url' id='wysihtml5-video' placeholder='https://www.youtube.com/watch?v=gaZDPUtSBZ0'/> <br >";         
         dlg(msg , { onAccept : insertVideo , AcceptText : "Insertar", ask : true});
